...
def conds(A, b):
    ...
    return (b_norm/x_norm*eff_sigma).item(), (b_norm/x_norm/original_numpy.abs(w[0])).item() # min_cond, eff_cond
...
class gradient_descent_mse_ensemble:
  ...
  def predict_inf(self, get: Get):
    ...
    '''if k_dd.nngp is not None:
        print('nngp condition number: %e -> %e'%(np.linalg.cond(k_dd.nngp), eff_cond(k_dd.nngp, self.y_train)))
  ...
  def get_kernels(self, get: Get, x_test: Optional[np.ndarray],
    ...
    if 'get_condition_number' in kernel_fn_test_test_kwargs:
        ...
        if 'nngp' in get_c and k_dd.nngp is not None:
          self.nngp_c = conds(k_dd.nngp, self.y_train)#, eff_cond(k_dd.nngp, self.y_train).item()
        if 'ntk' in get_c and k_dd.ntk is not None:
          self.ntk_c = conds(k_dd.ntk, self.y_train)#, eff_cond(k_dd.ntk, self.y_train).item()
    if x_test is None:
      k_td = None
