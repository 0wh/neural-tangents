...
def _fem1d(x1, x2):
  if x2 is None:
    ...
    cl = np.concatenate((np.array([1, -1/h[0]]), np.zeros((L.shape[0], 1))), axis=0)
    cr = np.concatenate((np.zeros((L.shape[0], 1)), np.array([-1/h[-1], 1])), axis=0)
